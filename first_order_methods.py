...
class First_Order_Methods:
    ...
    def universal_AC_FGM(self, k, L_0=None, gamma=None, beta=0.184, alpha=0.1, x_0=None, eps=0, initial_line_search=True, adaptive_restart=False):
        """
        ...
        adaptive restart: binary variable, decide whether to implement the adaptive restart rule.
    ...
    def FISTA(self, k, L_0=None, gamma=None, x_0=None, beta=2, adaptive_restart=False):
        """
        The FISTA method with backtracking in Beck and Teboulle (2009)
        ...
        gamma: scale of the regularizer, if applicable, usually set as prob.gamma
        x_0: the initialized search point
        beta: line search parameter. For the sake of consistency with UniversalFGM, we take the default value of beta as 2
        adaptive restart: binary variable, decide whether to implement the adaptive restart rule.
        ...
        for t in range(1, k+1):
            f_y, _, g_new = self.prob.first_order_oracle(y)
            oracles += 1
            for i in range(1000):
                L_new = beta**i * L
                x_new = self.prob.prox_mapping(y, 1/L_new * g_new, 1/L_new * gamma)
                f_x = self.prob.f(x_new)
                tmp = f_y + np.dot(g_new, x_new - y) + L_new/2 * (np.linalg.norm(y-x_new))**2 
                oracles += 1
                if f_x <= tmp + 1e-15 * f_y:
                    break
            L = L_new # L = L_new/2 (Nesterov's trick in universalFGM)

            # Add adaptive restart
            t_k_new = (1 + np.sqrt(1 + 4 * t_k**2))/2
            y_new = x_new + (t_k - 1)/t_k_new * (x_new - x)
            t_k = t_k_new
            obj_new = self.prob.objective(x_new)
            if adaptive_restart == True:
                # Adaptive restart rule
                if obj_new > obj:
                    t_k = 1
    ...
    def test_all_methods_nonsmooth(self, k, beta=0.184, L_0=None, x_0=None, eps=1e-8, initial_line_search=True, draw_performance_plot=False, dataset="",  right_lim=15000):
        ...
        if draw_performance_plot == True:
            ...
            plt.legend(["AC-FGM:0.5", "AC-FGM:0.1", "AC-FGM:0.0", "NS-PGM", "NS-FGM"], fontsize=16)
            plt.yscale("log")
            ...
            plt.ylabel("Error", fontsize=18)
            plt.xlabel("oracle calls", fontsize=18)
            plt.grid(linestyle= '--')
            plt.title(dataset, fontsize=22)
            plt.show()
    ...
    def test_all_methods_smooth(self, k, beta=0.184, L_0=None, x_0=None, initial_line_search=True, draw_performance_plot=False, dataset="", f_star=None, right_lim=15000, save_fig=False, save_title="plot", xsize = 15):
        ...
        results_FISTA = self.FISTA(k=k, gamma=self.prob.gamma)
        if f_star == None:
            f_star = min([min(results_AC_FGM[0]), min(results_AC_FGM_01[0]), min(results_AC_FGM_02[0]), min(results_adaptiveGD[0]), min(results_NS_FGM[0]), min(results_FISTA[0])])
        ...
        if draw_performance_plot == True:
            ...
            plt.plot(results_AC_FGM_01[0]-f_star, "y", linewidth=3)
            plt.plot(results_AC_FGM[0]-f_star, "m", linewidth=3)
            plt.plot(results_FISTA[0]-f_star, ":r", linewidth=3)
            plt.plot(results_adaptiveGD[0]-f_star, "-.g", linewidth=3)
            plt.plot(results_NS_FGM[0]-f_star, "--k", linewidth=3)
            plt.legend(["AC-FGM:0.5", "AC-FGM:0.1", "AC-FGM:0.0", "FISTA", "AdGD", "NS-FGM"], fontsize=16)
            plt.yscale("log")
            ...
            plt.plot(results_AC_FGM_01[2], results_AC_FGM_01[0]-f_star, "y", linewidth=3)
            plt.plot(results_AC_FGM[2], results_AC_FGM[0]-f_star, "m", linewidth=3)
            plt.plot(results_FISTA[2], results_FISTA[0]-f_star, ":r", linewidth=3)
        ...
        return [results_AC_FGM, results_AC_FGM_01, results_AC_FGM_02, results_adaptiveGD, results_NS_FGM, results_FISTA], f_star
    
    def test_all_methods_smooth_adaptive_restart(self, k, beta=0.184, L_0=None, x_0=None, initial_line_search=True, draw_performance_plot=False, dataset="", f_star=None, right_lim=15000, save_fig=False, save_title="plot", xsize = 15):
        """
        A simple function for testing the performance of different optimizer (with adaptive restart) in solving a smooth problem
        ...
        results_AC_FGM_01 = self.universal_AC_FGM(k=k, beta = beta, x_0=x_0, alpha=0.1, eps=0, gamma=self.prob.gamma, initial_line_search=initial_line_search)
        results_FISTA = self.FISTA(k=k, gamma=self.prob.gamma)
        results_FISTA_restart = self.FISTA(k=k, gamma=self.prob.gamma, adaptive_restart=True)
        results_AC_FGM_restart_01 = self.universal_AC_FGM(k=k, beta = beta, x_0=x_0, alpha=0.1, eps=0, gamma=self.prob.gamma, initial_line_search=initial_line_search, adaptive_restart=True)

        if f_star == None:
            f_star = min([min(results_AC_FGM_01[0]), min(results_FISTA[0]), min(results_FISTA_restart[0]), min(results_AC_FGM_restart_01[0])])
        ...
        if draw_performance_plot == True:
            ...
            plt.plot(results_AC_FGM_01[0]-f_star, "y", linewidth=3)
            plt.plot(results_FISTA[0]-f_star, ":r", linewidth=3)
            plt.plot(results_AC_FGM_restart_01[0]-f_star, "--", linewidth=3)
            plt.plot(results_FISTA_restart[0]-f_star, "-.", linewidth=3)
            plt.legend(["AC-FGM:0.1", "FISTA", "AC-FGM:0.1 (restart)", "FISTA (restart)"], fontsize=16)
            plt.yscale("log")
            plt.xticks(fontsize=xsize)
            plt.yticks(fontsize=15)
            plt.ylabel("Error", fontsize=18)
            ...
            plt.plot(results_AC_FGM_01[2], results_AC_FGM_01[0]-f_star, "y", linewidth=3)
            plt.plot(results_FISTA[2], results_FISTA[0]-f_star, ":r", linewidth=3)
            plt.plot(results_AC_FGM_restart_01[2], results_AC_FGM_restart_01[0]-f_star, "--", linewidth=3)
            plt.plot(results_FISTA_restart[2], results_FISTA_restart[0]-f_star, "-.", linewidth=3)
            plt.legend(["AC-FGM:0.1", "FISTA", "AC-FGM:0.1 (restart)", "FISTA (restart)"], fontsize=16)
            plt.yscale("log")
            plt.xticks(fontsize=xsize)
            plt.yticks(fontsize=15)
        ...
        return [results_AC_FGM_01, results_FISTA, results_AC_FGM_restart_01, results_FISTA_restart], f_star
        

def load_dataset_plot_nonsmooth(data, dataset="",  right_lim=15000, legend_size = 18):
    """
    For re-plotting with saved datasets (nonsmooth problems)
    """
    [results_AC_FGM, results_AC_FGM_01, results_AC_FGM_02, results_NS_PGM, results_NS_FGM], f_star = data
    plt.figure(figsize=(9,6.5),dpi=150)
    plt.plot(results_AC_FGM_02[0]-f_star, "b", linewidth=3)
    plt.plot(results_AC_FGM_01[0]-f_star, "y", linewidth=3)
    plt.plot(results_AC_FGM[0]-f_star, "m", linewidth=3)
    plt.plot(results_NS_PGM[0]-f_star, ":r", linewidth=3)
    plt.plot(results_NS_FGM[0]-f_star, "--k", linewidth=3)
    plt.legend(["AC-FGM:0.5", "AC-FGM:0.1", "AC-FGM:0.0", "NS-PGM", "NS-FGM"], fontsize=legend_size)
    plt.yscale("log")
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    plt.ylabel("Error", fontsize=18)
...
def load_dataset_plot_smooth(data, dataset="", right_lim=15000, xsize = 15, legend_size = 18):
    """
    For re-plotting with saved datasets (smooth problems)
    """
    [results_AC_FGM, results_AC_FGM_01, results_AC_FGM_02, results_adaptiveGD, results_NS_FGM, results_FISTA], f_star = data
    plt.figure(figsize=(9,6.5),dpi=150)
    plt.plot(results_AC_FGM_02[0]-f_star, "b", linewidth=3)
    plt.plot(results_AC_FGM_01[0]-f_star, "y", linewidth=3)
    plt.plot(results_AC_FGM[0]-f_star, "m", linewidth=3)
    ...
    plt.plot(results_adaptiveGD[0]-f_star, "-.g", linewidth=3)
    plt.plot(results_NS_FGM[0]-f_star, "--k", linewidth=3)
    plt.legend(["AC-FGM:0.5", "AC-FGM:0.1", "AC-FGM:0.0", "FISTA", "AdGD", "NS-FGM"], fontsize=legend_size)
    plt.yscale("log")
    plt.xticks(fontsize=xsize)
    plt.yticks(fontsize=15)
    plt.ylabel("Error", fontsize=18)
